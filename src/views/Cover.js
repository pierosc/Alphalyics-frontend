...
const [alldata, setAlldata] = useState([{date:"2007-01-03",indice_alphalytics:1,indice_sp500:1},{date:"2007-01-05",indice_alphalytics:1.04,indice_sp500:
1.068}])
    ...
    const getData2 = async () => {
        await axios.get('http://127.0.0.1:8000/api/v1/maindetail/ ').then((response) => {
            const data = response.data
            console.log(data)
            setAlldata(data)
        })
...
// getData2()
    ...
    const [userData, setUserData] = useState({
        ...
        //   data: [{x:'2016-12-25',y:2},{x:'2016-12-26',y:5},{x:'2016-12-28',y:7},{x:'2016-12-30',y:5}],
          data: [{x:1,y:2},{x:2,y:5},{x:3,y:7},{x:4,y:5}],
          backgroundColor:'rgba(0,0,0,1)',
          borderColor: 'rgba(0,0,0,1)',
          tension:0.4,
          normalized:true,
        ...
        //   data: [{x:'2016-12-25',y:8},{x:'2016-12-26',y:2},{x:'2016-12-28',y:4},{x:'2016-12-30',y:2}],
          data: [{x:1,y:2},{x:2,y:7},{x:3,y:5},{x:4,y:4}],
          backgroundColor:'rgba(128,128,128,1)',
          borderColor: 'rgba(128,128,128,1)',
          tension:0.4,
          normalized:true,
    ...
    //   rgba(140,197,140,1) VERDE OSCURO
      ...
      function getGradient(ctx, chartArea, scales){
        ...
        const percentage = (scales.x.getPixelForTick(3)-chartArea.left)/chartArea.width;
        gradientBg.addColorStop(0, 'rgba(250, 242, 249, 1)');
        gradientBg.addColorStop(0.10, 'rgba(250, 242, 249, 1)');
        gradientBg.addColorStop(0.10, 'rgba(226, 234, 221, 1)');
        gradientBg.addColorStop(0.15, 'rgba(226, 234, 221, 1)');
        gradientBg.addColorStop(0.15, 'rgba(252, 233, 220, 1)');
        gradientBg.addColorStop(0.50, 'rgba(252, 233, 220, 1)');
        gradientBg.addColorStop(0.50, 'rgba(255, 130, 130, 1)');
  ...
  return (
    ...
    <div>
    <img style={{ width: 700 }} className="h-auto  mb-8" src={require("./Imagen2.png")} alt="image description"></img>
    </div>
    <h2 className="text-4xl mb-6 font-bold ">Es un portafolio de inversión dinámico que invierte en activos del mercado de EEUU, buscando maximizar la rentabilidad a través del ciclo económico americano
</h2>
    <div className="flex flex-col justify-around">
     ...
     <div style={{ width: 1200 }} >
      ...
      <h3 className="text-3xl mb-6 font-bold w-2/7">Entre enero de 2007 y noviembre 2022 , el portafolio algorítmico de Alphalytics obtendría un retorno total de 16.4% vs 11.0% del Standard & Poor’s 500   
</h3>
